...
昨天提交B 
今天提交C 
使用 git revert B 命令，只回退B提交，而不会回退C提交, git 会生成一个新的 commit，将指定的 commit 内容从当前分支上撤除
...
在编译git库拉下来的代码时，往往会产生一些中间文件，这些文件我们根本不需要, 这些文件都是 Untracked files, 需要我们清除
...
6. 使用 git push -u origin master 将本地的master分支推送到origin主机, 同时 -u 指定origin为默认主机, 以后就可以直接使用 git push 了
