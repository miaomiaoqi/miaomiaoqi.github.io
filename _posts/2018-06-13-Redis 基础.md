...
隐患: 若当前的进程数据量庞大, fork之后数据量*2, 会造成服务器压力过大

#### 触发机制

**save(同步):** 在 redis 客户端中执行 save 命令, 有个致命的问题是这个命令是一个阻塞操作 O(n)

**bgsave(异步):** 在 redis 客户端中执行 bgsave 命令, redis 会使用 fork()函数, fork 一个子进程进行备份
...
| rdbcompression yes              | 是否采用压缩格式                  | yes                                   |
