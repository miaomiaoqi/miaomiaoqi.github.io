...
![http://www.milky.show/images/algorithm/stringmatch/sm_20.png](http://www.milky.show/images/algorithm/stringmatch/sm_20.png)
...
![http://www.milky.show/images/algorithm/stringmatch/sm_21.png](http://www.milky.show/images/algorithm/stringmatch/sm_21.png)

坏字符的位置越靠右, 下一轮模式串的挪动跨度就可能越长, 节省的比较次数也就越多.这就是BM算法从右向左检测的好处.

接下来, 我们继续逐个字符比较, 发现右侧的G、C、G都是一致的, 但主串当中的字符A, 是又一个坏字符: 

![http://www.milky.show/images/algorithm/stringmatch/sm_22.png](http://www.milky.show/images/algorithm/stringmatch/sm_22.png)

我们按照刚才的方式, 找到模式串的第2位字符也是A, 于是我们把模式串的字符A和主串中的坏字符对齐, 进行下一轮比较: 

![http://www.milky.show/images/algorithm/stringmatch/sm_23.png](http://www.milky.show/images/algorithm/stringmatch/sm_23.png)

接下来, 我们继续逐个字符比较, 这次发现全部字符都是匹配的, 比较公正完成: 

![http://www.milky.show/images/algorithm/stringmatch/sm_24.png](http://www.milky.show/images/algorithm/stringmatch/sm_24.png)

如果坏字符在模式串中不存在, 直接把模式串挪到主串坏字符的下一位即可

![http://www.milky.show/images/algorithm/stringmatch/sm_25.png](http://www.milky.show/images/algorithm/stringmatch/sm_25.png)
...
![http://www.milky.show/images/algorithm/stringmatch/sm_26.png](http://www.milky.show/images/algorithm/stringmatch/sm_26.png)

对于上面的例子, 如何我们继续使用“坏字符规则”, 会有怎样的效果呢？

从后向前比对字符, 我们发现后面三个字符都是匹配的, 到了第四个字符的时候, 发现坏字符G: 

![http://www.milky.show/images/algorithm/stringmatch/sm_27.png](http://www.milky.show/images/algorithm/stringmatch/sm_27.png)

接下来我们在模式串找到了对应的字符G, 但是按照坏字符规则, 模式串仅仅能够向后挪动一位: 

![http://www.milky.show/images/algorithm/stringmatch/sm_28.png](http://www.milky.show/images/algorithm/stringmatch/sm_28.png)

这时候坏字符规则显然并没有起到作用, 为了能真正减少比较次数, 轮到我们的好后缀规则出场了.由于好后缀规则的实现细节比坏字符规则要难理解得多, 所以我们这里只介绍一个大概思路: 

![http://www.milky.show/images/algorithm/stringmatch/sm_29.png](http://www.milky.show/images/algorithm/stringmatch/sm_29.png)

我们回到第一轮的比较过程, 发现主串和模式串都有共同的后缀“GCG”, 这就是所谓的“好后缀”.

如果模式串其他位置也包含与“GCG”相同的片段, 那么我们就可以挪动模式串, 让这个片段和好后缀对齐, 进行下一轮的比较: 

![http://www.milky.show/images/algorithm/stringmatch/sm_30.png](http://www.milky.show/images/algorithm/stringmatch/sm_30.png)

显然, 在这个例子中, 采用好后缀规则能够让模式串向后移动更多位, 节省了更多无谓的比较.

如果模式串中并不存在其他与好后缀相同的片段, 我们该怎么处理呢? 是不是可以直接把模式串挪到好后缀后?

![http://www.milky.show/images/algorithm/stringmatch/sm_31.png](http://www.milky.show/images/algorithm/stringmatch/sm_31.png)

这是不可取的, 我们不能直接挪动模式串到好后缀的后面, 还要先判断一种特殊情况: 模式串的前缀是否和好后缀的后缀相匹配, 免得挪过头

![http://www.milky.show/images/algorithm/stringmatch/sm_32.png](http://www.milky.show/images/algorithm/stringmatch/sm_32.png)
