...
## 常规
    ...
    shift + alt + s: 生成 Getter and Setter(Getter and Setter)
    shift + alt + o: 重写 toString方法(toString())
...
* String Manipulation
    ...
    alt + r: 删除字符串中的所有空格(Remove all spaces)
    alt + d: 删除多个空行(Remove empty lines)
    alt + x: 删除字符串两端的空白(Trim)
    alt + shift + x: 删除字符串的多个空格(Trim all spaces)
    ```
