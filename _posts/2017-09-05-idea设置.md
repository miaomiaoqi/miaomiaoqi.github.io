...
### 其他
    ...
    alt + 窗口数字: 快速打开小窗口(Tool Windows中设置)
    	alt + 1: Projects
    	alt + 2: Maven Projects
    	alt + 3: Run
    	alt + 4: Debug
    	alt + 5: TODO
    	alt + 6: Structure
    	alt + 7: RestServices(需要安装 RestfulToolkit 插件)
    	alt + 8: Hierarchy
    	alt + f: FindBugs-IDEA
    	clt + t: Terminal
...
* TranslationPlugin(设置快捷键 Translate)

    翻译插件, alt + shift + t

* AceJump(设置快捷键Activate AceJump mode)

    激活插件, 输入想要跳转的字符, 输入快捷跳转对应的字符 alt + ;
...
* codehelper.generator(设置快捷键 CodeHelper)
...
* RestfulToolkit(设置快捷键 RestfulToolkit -> Service)
    ...
    1. 方便我们根据 url 搜寻 controller, 在 KeyMap 中配置 RestfulToolkit 下的 Service 快捷键使用 command + alt + n
    2. 右边栏会多出 RestServices 方便我们接口测试

* iedis

    右边栏连接 redis 集群的客户端软件
