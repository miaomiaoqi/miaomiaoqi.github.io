...
1. 给 history 命令增加时间, .zshrc 中加入以下行
...
4. iterm2 添加 triggers
    ...
    添加步骤: command+“,” 组合键打开“Preferences”面板 -> Profiles 选项卡 -> Advanced -> Triggers(点击Edit即可)
...
1. 生成公私钥对
    ...
    在 ~/.ssh 目录下会生成两个文件 id_rsa(私钥)和 id_rsa.pub(公钥)
...
1. 配置别名
    ...
    如果使用的是 zsh, 可以修改配置文件指定 ssh 命令的别名快捷免密登录远程服务器
