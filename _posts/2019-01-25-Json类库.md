...
public class GsonDemo {
    ...
    public void testAlias() {
        // 以Teacher对象为例，Teacher中有一个aliasName的field
        // 假如，某一天，JSON字符串的变量名aliasName变成了fullName，我们不用把Teacher类里的变量aliasName改为fullName，然后把它的get和set方法都改了，然后把用到get/set方法的地方全改过来
        // 只需要用Gson提供的注解方法@SerializedName就行了
        GsonTeacher t = new GsonTeacher(1, "as", new Date(), false, null, null, "alias field", null);
        Gson gson = new Gson();
        String json = gson.toJson(t);
        System.out.println(json);
        // 这样虽然JSON字符串里的变量名为fullName，但是反序列化后fullName的值会映射给aliasName。同样，把一个Teacher对象序列化，变量aliasName会自动转换为fullName。
        t = gson.fromJson(json, GsonTeacher.class);

        System.out.println(t);
        // 然而现实远比想象中复杂，这个JSON有时候传的是fullName，有时候传的是aliasName，这时该怎么办呢？ 不用担心，@SerializedName 接受两个参数，value 和 alternate ，顾名思义，alternate 是备选变量名
        // 如果JSON传的是aliasName，那么就用aliasName的值，如果传的是fullName，那么就用fullName的值。需要注意的是，alternate只是反序列化JSON
        // 的一个备选变量名，它不会影响序列化，Teacer对象序列化后，会使用value定义的名称为变量名。
        String json2 = "{\"id\":1,\"name\":\"as\",\"flag\":false,\"fullName\":\"alias field\"}";
        t = gson.fromJson(json2, GsonTeacher.class);
        System.out.println(t);

        // 在定义了value和alternate的情况下，假如JSON同时传来了aliasName和fullName，Teacher的aliasName变量会接受哪个值呢？
        // 经过测试，它会都接受。这样也是很合理的，因为Gson会对JSON中的变量一个一个地去解析，既然它可以接受aliasName，也可以接受fullName
        // ，那么当同时传来这两个变量时，它就会分别把它们解析出来，并把值赋给Teacer对象中的aliasName变量。那么，aliasName变量的值就会是后解析的那个JSON变量的值，因为它会把前一个值覆盖掉。
...
// JSONObject对应Map
// JSONArray是集合, 里边每一个元素是JSONObject
public class FastJsonDemo {
    ...
    /**
     * 测试简单对象序列化
     * 1. 当某个变量值为null时，fastjson在序列化的时候直接把这个变量忽略了。
     *
     * @author miaoqi
     * @date 2019/1/31
     * @param
    ...
    /**
     * 测试简单对象反序列化
     * 1. 对于JSON字符串里没有的变量，fastjson在反序列化时会给它一个默认值，int类型默认为0，bool类型默认为false，String类型默认为null。如果JSON字符串里某个变量的值为null,
     * 和没有的结果是一样的。
     *
     * @author miaoqi
     * @date 2019/1/31
    ...
    /**
     * 格式化输出json, null作为空字符串输出(只对字符串类型有效), 使用单引号
     *
