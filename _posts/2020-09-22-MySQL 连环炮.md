...
数据库的最终数据是要落盘的，试想如果我想修改100张表里的10000条数据，如果每次修改都直接以落盘的形式处理，那么必定有大量的随机IO，如果使用机械磁盘那么一定存在大量的disk seek时间，数据库性能极低（我的另一篇文章：[从磁盘到文件系统](https://www.jianshu.com/p/a5d783643fc2) 很清晰的描述了为什么随机IO性能低）。为了能提高性能，我们希望能将这些改动缓存下来，统一处理，但问题是：如果这期间数据库崩溃了，那么这部分修改就丢失了，Durability就无法得到满足。
...
![http://www.milky.show/images/databases/mysql/mysql_3.png](http://www.milky.show/images/databases/mysql/mysql_3.png)
